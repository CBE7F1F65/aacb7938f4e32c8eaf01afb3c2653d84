#include "..\..\include\hge\hgeEffectSystem.h"
	...
	while(emitterItem)
	{
		...
		if(emitter->nAge > emitter->eei.nStartTime && emitter->nAge <= emitter->eei.nEndTime)
		{
			...
			while(emitter->fEmissionResidue >= 1.0f)
			{
				...
				_obj->fScaleX = 1.0f;
				_obj->fScaleY = 1.0f;
				_obj->nLifeTime = JGE::GetInstance()->Random_Int(emitter->eei.nLfieTimeMin, emitter->eei.nLifeTimeMax);
				float _x, _y, _z;
				if(emitter->eei.fThetaStep)
				{
					...
					_x = JGE::GetInstance()->Random_Float(-_r, _r);
					_y = JGE::GetInstance()->Random_Float(-_r, _r);
				...
				else
				{
					...
					{
						float _r = JGE::GetInstance()->Random_Float(emitter->eei.fRadiusInner, emitter->eei.fRadius);
						float _arc = JGE::GetInstance()->Random_Float(0, M_PI * 2);
						_x = _r * cosf(_arc);
						_y = _r * sinf(_arc);
					}
					else
					{
						_x = JGE::GetInstance()->Random_Float(-emitter->eei.fRadius, emitter->eei.fRadius);
						_y = JGE::GetInstance()->Random_Float(-emitter->eei.fRadius, emitter->eei.fRadius);
					}
				}
				_z = 0;
				////////////////////////ROTATIONX////////////////////////
...
//				D3DXVec3Normalize(&_vec3, &_vec3);
				...
				float _h = sqrtf(_vecx * _vecx + _objy * _objy + _objz * _objz);
				_vecx /= _h;
				_vecy /= _h;
				_vecz /= _h;
...
//					_obj3 += _vec3 * obj->fSpeedRaditial;
					_objx += _vecx * obj->fSpeedRaditial;
					_objy += _vecy * obj->fSpeedRaditial;
					_objz += _vecz * obj->fSpeedRaditial;
				}
...
//					_obj3 += _vec3 * obj->fSpeedTangential;
					float _tx = _vecy * _z - _vecz * _y;
					float _ty = _vecz * _x - _vecx * _z;
					float _tz = _vecx * _y - _vecy * _x;
					_vecx = _tx;
					_vecy = _ty;
					_vecz = _tz;
					_objx += _vecx * obj->fSpeedTangential;
					_objy += _vecy * obj->fSpeedTangential;
					_objz += _vecz * obj->fSpeedTangential;
				}
				obj->x = _objx;//_obj3.x;
				obj->y = _objy;//_obj3.y;
				obj->z = _objz;//_obj3.z;
...
void hgeEffectSystem::UpdateValue(float * value, float * bufferValue, hgeEffectAffectorInfo * eai, int nAge)
{
	...
	if(nAge == eai->nAffectorStartTime + 1)
	{
		*bufferValue = 0;
		...
		{
			*value = JGE::GetInstance()->Random_Float(eai->fStartValueMin, eai->fStartValueMax);
		}
	}
	...
	if(eai->bUseEndValue)
	{
		if(updateBuffer)
		{
			*bufferValue = JGE::GetInstance()->Random_Float(eai->fEndValueMin, eai->fEndValueMax);
		}
	...
	else
	{
		if (updateBuffer)
		{
			*bufferValue = JGE::GetInstance()->Random_Float(eai->fIncrementValueMin, eai->fIncrementValueMax);
		}
...
void hgeEffectSystem::UpdateColorValue(float * value, float * bufferValue, hgeEffectAffectorInfo * eai, int nAge)
{
	...
	if(nAge == eai->nAffectorStartTime + 1)
	{
		*(DWORD *)bufferValue = 0;
		...
		if (eai->bUseStartValue)
		{
			...
			_a = JGE::GetInstance()->Random_Int(GETA(scolmin), GETA(scolmax));
			_r = JGE::GetInstance()->Random_Int(GETR(scolmin), GETR(scolmax));
			_g = JGE::GetInstance()->Random_Int(GETG(scolmin), GETG(scolmax));
			_b = JGE::GetInstance()->Random_Int(GETB(scolmin), GETB(scolmax));
		}
	}
	...
	if(eai->bUseEndValue)
	{
		if(updateBuffer)
		{
			...
			_buffera = JGE::GetInstance()->Random_Int(GETA(ecolmin), GETA(ecolmax));
			_bufferr = JGE::GetInstance()->Random_Int(GETR(ecolmin), GETR(ecolmax));
			_bufferg = JGE::GetInstance()->Random_Int(GETG(ecolmin), GETG(ecolmax));
			_bufferb = JGE::GetInstance()->Random_Int(GETB(ecolmin), GETB(ecolmax));
		}		
		if(eai->nAffectorEndTime > nAge)
		{
	...
	else
	{
		if (updateBuffer)
		{
			...
			_buffera = JGE::GetInstance()->Random_Int(GETA(acolmin), GETA(acolmax));
			_bufferr = JGE::GetInstance()->Random_Int(GETR(acolmin), GETR(acolmax));
			_bufferg = JGE::GetInstance()->Random_Int(GETG(acolmin), GETG(acolmax));
			_bufferb = JGE::GetInstance()->Random_Int(GETB(acolmin), GETB(acolmax));
		}
